...
func main() {
	internalErrorCodes := map[string][]string{}
	changedErrorCodesStat := map[string]int{}

	// TODO:- Step1 extract MozartId
	csv_file, err := os.Open(getFullPath("all_error.csv"))
	if err != nil {
		panic(err)
	}
	...
	for _, record := range records {
		...
		if paymentID != "" && internalErrorCodes[paymentID] == nil {
			internalErrorCodes[paymentID] = []string{}
		}

		internalErrorCodes[paymentID] = append(internalErrorCodes[paymentID], record[0])
	...
	for paymentId, val := range internalErrorCodes {
		...
		n := len(val)
		initialInternalErrorCode := val[n-1]
		...
		for i := n - 2; i >= 0; i-- {
			...
			if val[i] != initialInternalErrorCode {
				if initialInternalErrorCode == "BAD_REQUEST_PAYMENT_UPI_COLLECT_REQUEST_REJECTED" {
					fmt.Println(paymentId, "### ", val[i], " <====", initialInternalErrorCode)
				}
